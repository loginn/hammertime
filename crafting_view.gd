...
func update_hammer_button_states():
...
func _on_finish_item_button_pressed():
	finish_item()
...
func set_new_item_base(item_base: Item):
...
func add_hammers(implicit_count: int, prefix_count: int, suffix_count: int):
	...
	print("Hero found hammers! Added: ", implicit_count, " implicit, ", prefix_count, " prefix, ", suffix_count, " suffix")
	print("Total hammers now: Implicit(", hammer_counts["implicit"], ") Prefix(", hammer_counts["prefix"], ") Suffix(", hammer_counts["suffix"], ")")
	
	# Update button states to reflect new hammer counts
	update_hammer_button_states()

func initialize_crafting_inventory():
...
func _on_item_type_selected(item_type: String):
...
func update_item_type_button_states():
