...
@onready var _tab_totem: Button = %TabTotem
...
@onready var _totem_screen: Control = $VBox/Body/TotemScreen
@onready var _settings_screen: Control = $VBox/Body/SettingsScreen
...
func _ready() -> void:
	...
	_tab_totem.pressed.connect(_on_tab_totem_pressed)
	_tab_settings.pressed.connect(_on_tab_settings_pressed)

	GameEvents.prestige_completed.connect(_update_totem_tab_visibility)

	_update_totem_tab_visibility()
...
func _on_tab_totem_pressed() -> void:
	_switch_to_view("totem")


func _on_tab_settings_pressed() -> void:
	_switch_to_view("settings")


func _update_totem_tab_visibility() -> void:
	_tab_totem.visible = PrestigeManager.prestige_count >= 2
