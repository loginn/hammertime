# Requirements Archive: v0.1 Code Cleanup & Architecture
...
- [x] **STYLE-01**: All GDScript files are formatted with gdformat (consistent style)
- [x] **STYLE-02**: All files and classes follow snake_case naming convention (Godot 4 standard)
- [x] **STYLE-03**: Function signatures have return type hints

### File Organization

- [x] **ORG-01**: Project uses feature-based folder structure (models/, scenes/, autoloads/, utils/)
- [x] **ORG-02**: All .gd and .tscn files are moved from root to appropriate folders
- [x] **ORG-03**: File moves preserve all scene references (done via Godot Editor)
...
- [x] **CALC-01**: Single unified stat calculation system replaces duplicate compute_dps() in weapon.gd and ring.gd
- [x] **CALC-02**: Tag system is separated into AffixTag (affix filtering/eligibility) and StatType (damage calculation routing)
- [x] **CALC-03**: All item types use a standardized update_value() interface
- [x] **CALC-04**: Damage calculation inconsistencies are resolved (multiplicative vs additive, crit formula differences)

### View Communication

- [x] **VIEW-01**: Views use signals instead of direct get_node() calls for cross-view communication
- [x] **VIEW-02**: Parent-child communication follows "call down, signal up" pattern
- [x] **VIEW-03**: Node references use @onready caching instead of repeated get_node() calls
...
|-------------|-------|--------|
| STYLE-01 | Phase 1 | Done |
| STYLE-02 | Phase 1 | Done |
| STYLE-03 | Phase 1 | Done |
| ORG-01 | Phase 1 | Done |
| ORG-02 | Phase 1 | Done |
| ORG-03 | Phase 1 | Done |
...
| CALC-01 | Phase 3 | Done |
| CALC-02 | Phase 3 | Done |
| CALC-03 | Phase 3 | Done |
| CALC-04 | Phase 3 | Done |
| VIEW-01 | Phase 4 | Done |
| VIEW-02 | Phase 4 | Done |
| VIEW-03 | Phase 4 | Done |
